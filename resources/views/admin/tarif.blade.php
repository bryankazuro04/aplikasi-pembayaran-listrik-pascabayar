...
@section('main')
  <div class="p-4 sm:p-6">
            ...
            <div class="p-4 border-b border-gray-200 hover:bg-gray-50">
              <div class="flex justify-between items-center mb-2">
                ...
                <div class="flex space-x-2">
                  <button onclick="editTarif({{ $tarif->id }}, '{{ $tarif->daya }}', '{{ $tarif->tarifperkwh }}')" class="text-blue-600 hover:text-blue-900">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <tr class="hover:bg-gray-50">
                ...
                <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-600">
                  <div class="flex space-x-3">
                    <button onclick="editTarif({{ $tarif->id }}, {{ $tarif->daya }}, {{ $tarif->tarif_per_kwh }})" class="text-blue-600 hover:text-blue-900">
                      <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                          d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                      </svg>
          ...
          <form action="{{ route('tarif.store') }}" method="POST">
            ...
            <div class="space-y-4">
              ...
              <div>
                ...
                <input type="number" name="tarifperkwh" required placeholder="Contoh: 1500" step="0.01"
                  class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm 
                         focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">
              </div>
            </div>
    ...
    <!-- Modal Edit Tarif -->
    <div id="editTarifModal" class="fixed inset-0 hidden h-full w-full overflow-y-auto bg-gray-600/40 p-4">
      <div class="flex min-h-screen items-center justify-center">
        <div class="relative w-full max-w-md rounded-lg border border-gray-200 bg-white p-6 shadow-xl">
          <div class="mb-4">
            <h3 class="text-lg font-medium leading-6 text-gray-900">Edit Tarif</h3>
            <button onclick="document.getElementById('editTarifModal').classList.add('hidden')"
              class="absolute right-4 top-4 text-gray-400 hover:text-gray-500">
              <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
              </svg>
          ...
          <form id="editTarifForm" method="POST">
            ...
            <div class="space-y-4">
              <div>
                <label class="block text-sm font-medium text-gray-700">Daya (VA)</label>
                <input type="number" id="edit_daya" name="daya" required
                  class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm 
                         focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">
              </div>
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700">Tarif per kWh (Rp)</label>
                <input type="number" id="edit_tarifperkwh" name="tarif_per_kwh" required step="0.01"
                  class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm 
                         focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500">
              </div>
            
            <div class="mt-6 flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-3 gap-2">
              <button type="button" onclick="document.getElementById('editTarifModal').classList.add('hidden')"
                class="inline-flex justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium 
                       text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 
                       focus:ring-offset-2">
                Batal
  ...
  @push('scripts')
    <script>
      function editTarif(id, daya, tarif_per_kwh) {
        document.getElementById('editTarifForm').action = `/tarif/${id}`;
        document.getElementById('edit_daya').value = daya;
        document.getElementById('edit_tarifperkwh').value = tarif_per_kwh;
