    ...
    <div class="overflow-hidden rounded-xl bg-white shadow-lg">
      ...
      <div class="border-b bg-gray-50 px-4 py-4 sm:px-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h2 class="text-xl sm:text-2xl font-bold text-gray-800">Tambah Pelanggan Baru</h2>
      ...
      <form action="{{ route('pelanggan.store') }}" method="POST" class="p-4 sm:p-6">
              ...
              <div class="space-y-2">
                ...
                <div class="relative rounded-md shadow-sm">
                  <input type="text" id="nomor_meter" name="nomor_meter"
                    class="block w-full rounded-lg border border-gray-300 px-4 py-2.5 text-gray-900 placeholder-gray-400 
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
              ...
              <div class="space-y-1 md:col-span-2">
                ...
                <select id="id_tarif" name="id_tarif"
                  class="block w-full rounded-lg border border-gray-300 px-4 py-2.5 text-gray-900 
                         focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
                  required>
                  <option value="">-- Pilih Tarif --</option>
                  @foreach ($tarif ?? [] as $t)
                    <option value="{{ $t->id }}">
                      {{ number_format($t->daya) }} VA - Rp{{ number_format($t->tarifperkwh, 0, ',', '.') }}/kWh
                    </option>
                  @endforeach
                </select>
        ...
        <div class="mt-8 flex flex-col-reverse gap-3 border-t pt-6 sm:flex-row sm:justify-end sm:space-x-4">
          <button type="reset"
            class="inline-flex justify-center items-center rounded-lg border border-gray-300 bg-white px-6 py-2.5 text-sm font-medium 
                   text-gray-700 shadow-sm transition-colors duration-200 hover:bg-gray-50 focus:outline-none 
          ...
          <button type="submit"
            class="inline-flex justify-center items-center rounded-lg bg-blue-600 px-6 py-2.5 text-sm font-medium text-white 
                   transition-colors duration-200 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 
                   focus:ring-offset-2">
            <svg class="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
